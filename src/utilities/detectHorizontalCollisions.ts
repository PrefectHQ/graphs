...
export async function clusterHorizontalCollisions({ items, createCluster }: ClusterHorizontalCollisionsProps): Promise<void> {
  const visibleItems = [...items.values()].sort((itemA, itemB) => itemA.element.x - itemB.element.x)
  ...
  async function clusterItems(
    ...
    const prevIds = itemIsClusterFactory(prevItem) ? prevItem.getIds() : [prevItem.getId()]
